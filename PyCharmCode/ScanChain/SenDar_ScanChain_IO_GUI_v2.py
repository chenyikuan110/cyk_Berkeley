...
def main():
    ...
    if socat_process or os.name == 'nt':
        try:
            # Main program logic here
            if socat_process:
                while True:
                    if os.path.exists(port):
                        print(f"Success: {port} is available.")
                        break  # Exit the function once the port is found
                    else:
                        print(f"Waiting for {port} to create by socat...")
